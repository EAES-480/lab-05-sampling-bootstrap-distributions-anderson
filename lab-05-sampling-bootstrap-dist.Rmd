...
> If you increase `n_samp` SE decreases because variability of the mean shrinks with larger samples. If you increase `reps` SE does not change. It does however make the estimate of the sampling distribution more precise. So, sample size affects the estimate, while the number of preps affects how well the simulation shows the distribution. 

> GM: -0.5, careful, yes, there is an increase in precision of the bootstrap distribution with increasing samples, but for `reps` increasing bootstrap reps makes the boostrap distribution more closely resemble a normal distribution.  Also, was hoping you would comment on the similarity of `se_theory` and `se_simulated`.
...
> The bootstrap distribution is centered close to the population mean, which occurs when the sample itself is representative of the underlying population. The bootstrap SD represents an estimate of the standard error of the sample mean, quantifying the uncertainty in the population mean based on the one sample. 

> GM: -0.25 - careful, the bootstrap SD is an estimate of the **standard error of the sampling distribution of the population parameter** in this case, the population mean, not the sample mean.
...
df_summer <- df %>%
  filter(month(date, label = TRUE, abbr = TRUE) %in% c("Jun", "Jul", "Aug")) %>%
  filter(!is.na(SWC))
...
> Yes, the bootstrap distribution looks narrow, even when centered away from the population mean. 

> The bootstrap measures the variability of estimates within the chosen sample, and not how similar or representative the sample is of the population. Precision refers to how consistent the estimates are, while accuracy refers to how close the estimate is to the actual population value. 

> Summer-only sampling introduces sampling bias, since we are excluding information that could be received by sampling in the other seasons. 
...
> The random and stratified sampling seem to produce the best bootstrap distributions, as they are closer to the population mean. The summer-only bootstrap has a narrow spread, making it seem precise, but it excludes other seasons which makes it inaccurate. The stratified sampling is the most accurate because observations from all months are represented equally. 

> This exercise shows that bootstrapping measures uncertainty within a sample, but the biased sampling designs still influence the overall distributions. 
...
**Prompt:** Does the confidence interval contain the **true population mean (`pop_mean_swc`)**?  
What does this tell you about the uncertainty of the estimate?

> The confidence interval (95%) does contain the true population mean, which indicates that the interval shows the uncertainty that is associated with the estimation of the mean from the sample.

> GM: good answer, but you can also say that the uncertainty of the estimate is good. the phrasing of the question is weak though, so I won't mark down here.
...
**Prompt:** Explain what the **red vertical lines** represent.

> The red vertical lines represent the 2.5th and 97.5th percentiles of the distribution. These values mark the range of sample means that contain the 95% of central bootstrap estimates.
...
```{r ex3_ci_params, echo=TRUE, eval=F}
quantile(boot_coef[,1], probs = c(0.025, 0.975))  # intercept CI
quantile(boot_coef[,2], probs = c(0.025, 0.975))  # slope CI
```

**Prompt:** Interpret the slope confidence interval.
- Does it include zero?
- What does that imply about the relationship between the predictor and response variable?

> The slope confidence interval does not include zero, which suggests that the predictor variable has an influential relationship with the response variable. The negative slope indicates that the predictor variable increases and the response decreases.

> GM: -0.25, careful here: "the confidence interval of the slope does not include zero, which supports a statistically significant relationship betwen x and y."
